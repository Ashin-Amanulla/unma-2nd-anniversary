...
import InternationalPaymentDialog from "./InternationalPaymentDialog";
...
const AlumniRegistrationForm = ({ onBack, storageKey }) => {
  ...
  const [showFinancialDifficultyDialog, setShowFinancialDifficultyDialog] =
    useState(false);
  const [showInternationalPaymentDialog, setShowInternationalPaymentDialog] =
    useState(false);
  ...
  // Add effect to handle auto-scroll when step changes
  useEffect(() => {
    // Scroll to top when step changes
    const scrollToTop = () => {
      // Use requestAnimationFrame and a small delay to ensure DOM is updated and layout is stable
      requestAnimationFrame(() => {
        setTimeout(() => {
          try {
            // First try to scroll to the specific scroll target
            const scrollTarget = document.getElementById("form-scroll-target");
            if (scrollTarget) {
        ...
        }, 50); // Small delay to allow for layout changes
      });
    };
  ...
  // Add effect to prevent unwanted redirects during registration
  useEffect(() => {
    // Store original location
    const originalPushState = window.history.pushState;
    const originalReplaceState = window.history.replaceState;

    // Override history methods to prevent admin redirects
    const preventAdminRedirect = (original) => {
      return function (state, title, url) {
        // If trying to redirect to admin login during registration, prevent it
    ...
    window.history.pushState = preventAdminRedirect(originalPushState);
    window.history.replaceState = preventAdminRedirect(originalReplaceState);
  ...
  const validateCurrentStep = async () => {
    ...
    if (currentStep === 0) {
      ...
      // If "JNV Other" is selected, also validate customSchoolName
      // If state is Kerala, district is required
      const currentSchool = watch("school");
      if (currentSchool === "JNV Other") {
        fieldsToValidate.push("customSchoolName");

        // Check if district is actually selected
        const currentSchoolName = watch("customSchoolName");
      ...
      const profession = watch("profession");
      if (profession && Array.isArray(profession) && profession.length > 0) {
        if (profession.includes("Business Owner/Entrepreneur")) {
    ...
    if (isStepValid) {
      ...
      if (currentStep > 0) {
        ...
        try {
          ...
          if (!saveSuccess) {
            // Show user-friendly message and don't proceed
            toast.error(
          ...
          console.error("Unexpected error in handleNextStep:", saveError);

          // Prevent any potential redirects by handling the error locally
      ...
      // Remove the immediate scroll since useEffect will handle it
      // window.scrollTo(0, 0);
    }
  };
  ...
  const saveStepToBackend = async (stepNumber, formData) => {
    try {
      ...
      // Prevent redirect to admin login for registration form errors
      // Handle specific error cases without triggering global error handlers
      if (error.response?.status === 401) {
        // Authentication error - but don't redirect to admin login
        toast.warning(
      ...
      // Handle other errors gracefully without redirect
      if (error.response?.data?.message) {
        ...
        toast.error(
          "Failed to save this step. Please check your connection and try again."
        );
      }
  ...
  const onSubmit = async (data) => {
    try {
      ...
      // Show success message based on registration status and payment status
      if (data.paymentStatus === "foreign-transaction") {
        toast.info("Registration submitted for verification");
        navigate("/registration-pending-success", {
          state: {
            registrationData: {
              name: data.name,
              email: data.email,
      ...
      } else if (data.registrationStatus === "incomplete") {
        ...
        navigate("/registration-success?type=alumni", {
          state: {
            registrationData: {
              name: data.name,
              email: data.email,
  ...
  const handleSubmitForm = async () => {
    if (
      watch("paymentStatus") !== "Completed" &&
      watch("paymentStatus") !== "financial-difficulty" &&
      watch("paymentStatus") !== "foreign-transaction"
    ) {
      toast.error(
    ...
    await initiatePayment({
      ...
      onSuccess: async (response) => {
        ...
        try {
          ...
          setValue("contributionAmount", contributionAmount);

          // Show success toast
          toast.success(`Payment of ₹${contributionAmount} successful!`);

          // Save the payment info to backend
          // await saveStepToBackend(8, getValues());

          try {
            await onSubmit(getValues());
          } catch (submitError) {
            console.error("Auto-submission error:", submitError);
            toast.error(
              "Payment successful but form submission failed. Please click Submit Registration."
            );
          }
        } catch (error) {
  ...
  const handlePayment = async () => {
    try {
      ...
      // Check for financial difficulty
      {
        const attendees = watch("attendees");
        ...
        //toddler/teen cannot book alone, they need to book with an adult
        if (teenCount > 0 && adultCount === 0) {
          toast.error(
            "Toddler/teen cannot book alone, they need to book with an adult"
          );
          return;
  ...
  // Add the handleInternationalPayment function
  const handleInternationalPayment = async (paymentData) => {
    try {
      setIsSubmitting(true);

      // Update form data with international payment details
      setValue("paymentStatus", "foreign-transaction");
      setValue("contributionAmount", paymentData.amount);
      setValue(
        "paymentDetails",
        JSON.stringify({
      ...
      setValue("registrationStatus", "complete");
      ...
      console.error("International payment submission error:", error);
      toast.error("Registration submission failed. Please try again.");
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <>
      ...
      <form ref={formRef} className="space-y-4 sm:space-y-6">
        {/* Scroll target for mobile navigation */}
        <div id="form-scroll-target" className="scroll-mt-20" />
        ...
        {currentStep === 0 && (
          <>
              ...
              <div className="flex items-start">
                ...
                <div className="ml-3">
                  <h3 className="text-sm font-medium text-amber-800">
                    <strong>
                      A Note for Alumni from JNV Alappuzha and JNV Malappuram
                    </strong>
                  </h3>
                  <div className="mt-2 text-sm text-amber-700">
                    <p>
                      The registration process of JNV Alappuzha and JNV
                      Malappuram are routed through the respective alumni
                      ...
                      If any alumni from these schools wish to register directly
                      through the UNMA SUMMIT 2025 Portal, please select the{" "}
                      <strong>JNV Other </strong> option during registration.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <FormSection title="Contact Verification">
              ...
              {email && email.includes("@") && watch("contactNumber") && (
                <OtpInput
            ...
            <FormSection title="Navodayan Quiz (take a trip down memory lane)">
              {!quizCompleted && (
        ...
        {currentStep === 2 && (
          <FormSection title="Professional Information">
            <FormField
              ...
              placeholder="🔍 Type to search existing professions or create new ones..."
              helperText="Select from existing options or type to create new professions. You can select multiple if applicable."
              isSearchable={true}
              isClearable={false}
              filterOption={(option, inputValue) => {
                if (!inputValue) return true;
                ...
                // Enhanced search - search by profession name (starts with or contains)
                return (
                  label.startsWith(searchText) ||
                  label.includes(searchText) ||
                  value.startsWith(searchText) ||
                  // Also search within words
              ...
              noOptionsMessage={({ inputValue }) =>
                inputValue
                  ? `No professions found matching "${inputValue}". Press Enter to create it.`
                  : "No professions available"
              }
              // Enhanced styling for better search UX
              styles={{
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                <div className="flex items-start">
                  ...
                  <div className="ml-3">
                    ...
                    <div className="mt-2 text-sm text-blue-700">
                      <p>
                        The expected cost per attendee for UNMA Summit 2025 is
                        <strong>₹1300/-</strong> with part of the budget funded
                        through sponsorships and donations. We anticipate a
                        budget of <strong>₹40 lakhs</strong> for over{" "}
                        <strong>3,000</strong> attendees.
                      </p>
                      <p className="mt-2">
                        Your <strong>generous contributions</strong> will help
                        cover essential expenses such as{" "}
                        <strong>venue, food, logistics,</strong> and{" "}
                        <strong>financial assistance</strong> for alumni in
                        need. We’re <strong>waiving registration fees</strong>{" "}
                        for those requiring support and offering{" "}
                        <strong>discounted rates</strong> for the 2022 to 2025
                        batches.
                      </p>
                      <p className="mt-2">
                        By contributing, you're helping create an inclusive,
                        memorable event—and empowering future UNMA initiatives.
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <AlertDialog
              ...
              message={
                <div className="space-y-4 text-sm max-h-96 overflow-y-auto">
                  <p>
                    All attendees must pay a minimum fee based on total
                    attendees. Appreciate paying more to the best of your
                    capacity to support all of UNMA's future activities and to
                    cover alumni who are requesting payment exception. If you
                    cannot pay the minimum, contact your alumni leadership to
                    discuss options.
                  </p>
                </div>
              }
      ...
      {/* Mission Message Popup */}
      <InternationalPaymentDialog
        isOpen={showInternationalPaymentDialog}
        onClose={() => setShowInternationalPaymentDialog(false)}
        onSubmit={handleInternationalPayment}
        isSubmitting={isSubmitting}
      />
    </>
  );
